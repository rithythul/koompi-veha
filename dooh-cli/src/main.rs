...
use dooh_core::{Decoder, OutputSink, Player};
use dooh_output::WindowSink;

// ── CLI argument structs ────────────────────────────────────────────────

#[derive(Parser)]
#[command(name = "dooh", about = "koompi-dooh media player")]
struct Cli {
...
fn play(source: &str, width: Option<u32>, height: Option<u32>) -> dooh_core::Result<()> {
    ...
    let mut sink = WindowSink::new(&format!("dooh - {source}"), w, h)?;
...
fn play_playlist(
    ...
    let playlist = dooh_core::Playlist::from_json_file(playlist_path)?;
    ...
    let mut sink = WindowSink::new(&format!("dooh - {}", playlist.name), w, h)?;
