...
pub async fn resolve_for_board(
    ...
    // Board struct doesn't have sell_mode yet (Task 11 adds it).
    // For now, check the raw column via a direct query.
    let sell_mode: Option<(String,)> = sqlx::query_as(
        "SELECT sell_mode FROM boards WHERE id = ?"
    )
    .bind(board_id)
    .fetch_optional(pool)
    .await?;

    let sell_mode = sell_mode.map(|r| r.0).unwrap_or_else(|| "house_only".to_string());
    ...
    let zone_ids: Vec<String> = {
        let zone_id: Option<(Option<String>,)> = sqlx::query_as(
            "SELECT zone_id FROM boards WHERE id = ?"
        )
        .bind(board_id)
        .fetch_optional(pool)
        .await?;

        match zone_id.and_then(|r| r.0) {
