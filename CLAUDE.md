# CLAUDE.md — koompi-dooh

## Project Overview

koompi-dooh is a Rust Digital Out-of-Home (DOOH) advertising platform for LED billboard fleet management. It wraps FFmpeg for media playback and provides a full advertising workflow: zones, advertisers, campaigns, bookings, schedule resolution, and play-log analytics. It's a Cargo workspace with 7 crates.
...
cargo build -p dooh-player --features framebuffer  # with framebuffer support
cargo check -p dooh-web --target wasm32-unknown-unknown  # check WASM crate
```
...
```
koompi-dooh/
├── dooh-core/           # Library: FFmpeg decoder, frames, playlist, player, commands
│   └── src/
...
├── dooh-output/         # Library: Output backends
│   └── src/
...
├── dooh-agent/          # Binary: Board agent
│   └── src/
│       ├── main.rs      # Entry point
│       ├── config.rs    # TOML config (AgentConfig)
│       ├── ws_client.rs # WebSocket client with auto-reconnect
│       └── player_client.rs  # Unix socket IPC client to dooh-player
└── dooh-web/            # WASM: Browser playlist player
    ├── src/lib.rs       # wasm-bindgen DoohPlayer (no dooh-core dep)
...
- **dooh-player uses blocking thread + async tokio** — FFmpeg is synchronous, IPC is async. Shared state via `Arc<Mutex<>>`.
- **dooh-web does NOT depend on dooh-core** — FFmpeg can't compile to WASM. The WASM crate reimplements playlist/timing only, using browser-native `<video>` and `<img>`.
- **WsMessage is duplicated** in dooh-api and dooh-agent (not shared) to avoid circular dependencies.
...
- Error type is `dooh_core::Error` (thiserror), propagated via `dooh_core::Result<T>`
...
**Add a new output backend:** Create `dooh-output/src/mybackend.rs`, implement `OutputSink` trait, add feature gate in Cargo.toml and lib.rs, add match arm in dooh-player/src/main.rs.

**Add a new API endpoint:** Add handler in `dooh-api/src/routes.rs`, add DB function in `db.rs` if needed, add model in `models.rs`, register route in `create_router()`.

**Add a new player command:** Add variant to `PlayerCommand` enum in `dooh-core/src/command.rs`, handle it in dooh-player/src/main.rs command processing loop, update WsMessage in both dooh-api/src/ws.rs and dooh-agent/src/ws_client.rs.

## Test Fixtures

Test media files are in `dooh-core/tests/fixtures/`:
