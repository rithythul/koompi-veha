...
#[derive(Parser)]
#[command(name = "dooh-agent", about = "koompi-dooh board agent — bridges the API server and local player")]
struct Args {
    /// Path to agent config file (TOML)
    #[arg(short, long, default_value = "dooh-agent.toml")]
    config: String,
}
...
async fn main() {
    ...
    info!(
        "Starting dooh-agent board_id={} api={}",
