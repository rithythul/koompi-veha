...
/// A media item in the playlist (mirrors dooh-core::MediaItem but WASM-compatible).
...
/// A playlist (mirrors dooh-core::Playlist but WASM-compatible).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Playlist {
    pub name: String,
...
/// Dispatch a custom `dooh-advance` event on the document so that JS glue
...
fn dispatch_advance_event(document: &web_sys::Document) {
    ...
    if let Ok(event) = CustomEvent::new_with_event_init_dict("dooh-advance", &init) {
