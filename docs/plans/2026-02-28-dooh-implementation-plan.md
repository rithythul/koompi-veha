# veha Platform — Phase 1 Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Transform koompi-veha from a media player fleet manager into a veha (Digital Out-of-Home) advertising platform with zones, advertisers, campaigns, bookings, proof-of-play, and session authentication.

**Architecture:** Extend the existing axum + SQLite + WebSocket stack. Add 7 new DB tables via migration, session-based auth middleware, veha CRUD endpoints, a server-side schedule resolution engine that pushes resolved playlists to agents, and a revamped vanilla JS dashboard with 5 new pages.

**Tech Stack:** Rust (axum 0.8, sqlx 0.8, argon2, rand), SQLite, vanilla JS + Tailwind CDN.

**Design doc:** `docs/plans/2026-02-28-veha-platform-design.md`
...
**Files:**
- Create: `veha-api/migrations/003_veha.sql`
- Modify: `veha-api/src/db.rs` (add MIGRATION_003_SQL constant + execution)

**Step 1: Write migration SQL**

Create `veha-api/migrations/003_veha.sql` with:
...
-- Extend boards with veha fields (SQLite ADD COLUMN is safe for existing rows)
...
In `veha-api/src/db.rs`, after the `MIGRATION_002_SQL` execution block:
...
```bash
git add veha-api/migrations/003_veha.sql veha-api/src/db.rs
git commit -m "feat(api): add veha database schema — zones, advertisers, campaigns, bookings, play_logs, users"
```
...
**Files:**
- Modify: `veha-api/Cargo.toml` (add argon2, rand dependencies)
- Create: `veha-api/src/auth.rs` (auth module: hashing, sessions, middleware)
- Modify: `veha-api/src/models.rs` (add User, Session, LoginRequest, AuthResponse models)
- Modify: `veha-api/src/db.rs` (add user/session CRUD, default admin seeding)
- Modify: `veha-api/src/routes.rs` (add auth routes, apply middleware)
- Modify: `veha-api/src/main.rs` (add `mod auth`, seed default admin after DB init)

**Step 1: Add dependencies**

In `veha-api/Cargo.toml`, add:
```toml
argon2 = "0.5"
rand = "0.8"
```

**Step 2: Create auth module (`veha-api/src/auth.rs`)**
...
In `veha-api/src/db.rs`, add a new section:
...
In `veha-api/src/routes.rs`, add login/logout/me handlers:
...
fn extract_session_token(headers: &HeaderMap) -> Option<&str> {
    headers.get("cookie")
        .and_then(|v| v.to_str().ok())
        .and_then(|cookies| cookies.split(';').find_map(|c| c.trim().strip_prefix("veha_session=")))
}
```
...
In `veha-api/src/main.rs`, after `db::init_db()`:
...
**Files:**
- Modify: `veha-api/src/models.rs` (Zone, CreateZone models)
- Modify: `veha-api/src/db.rs` (zone CRUD functions)
- Modify: `veha-api/src/routes.rs` (zone handlers + route registration)
...
**Files:**
- Create: `veha-api/src/resolver.rs` (schedule resolution logic)
- Modify: `veha-api/src/db.rs` (add query functions for resolution)
- Modify: `veha-api/src/routes.rs` (add resolved-schedule preview endpoint)
- Modify: `veha-api/src/main.rs` (add `mod resolver`)
...
**Step 2: Create resolver module (`veha-api/src/resolver.rs`)**

```rust
use crate::db;
use crate::models::*;
use veha_core::{MediaItem, Playlist};
use sqlx::SqlitePool;
...
cargo build --workspace && cargo test --workspace
git add veha-api/
git commit -m "feat(api): add schedule resolution engine for veha bookings"
```
...
**Files:**
- Modify: `veha-api/src/ws.rs` (add ScheduleUpdate, PlayReport message types)
- Modify: `veha-agent/src/ws_client.rs` (add matching message types, handle ScheduleUpdate)
- Modify: `veha-core/src/command.rs` (extend PlayerStatus with optional veha fields)

**Step 1: Extend WsMessage on both sides**

Add to the WsMessage enum in both `veha-api/src/ws.rs` and `veha-agent/src/ws_client.rs`:
...
In `veha-core/src/command.rs`, add optional fields (backward compatible via serde defaults):
...
In `veha-agent/src/ws_client.rs`, in `handle_server_message()`:
...
In `veha-api/src/ws.rs`, in the message receive loop:
...
```bash
cargo build --workspace && cargo test --workspace
git add veha-api/ veha-agent/ veha-core/
...
**Files:**
- Modify: `veha-api/src/db.rs` (insert_play_log, list_play_logs, play_log_summary)
- Modify: `veha-api/src/models.rs` (PlayLog, PlayLogFilter, PlayLogSummary)
- Modify: `veha-api/src/routes.rs` (play log list + summary endpoints)
...
**Files:**
- Modify: `veha-api/static/app.js` (fix existing pages for paginated responses, add new page functions)
- Modify: `veha-api/static/index.html` (add new page sections + sidebar nav items)
- Modify: `veha-api/static/style.css` (any new styles needed)
...
**Files:**
- Modify: `veha-api/src/models.rs` (UpdateBoard model with veha fields)
- Modify: `veha-api/src/db.rs` (update_board function, extend Board struct query columns)
- Modify: `veha-api/src/routes.rs` (PUT /api/boards/{id} handler, extend list_boards filters)
- Modify: `veha-api/static/app.js` (enhanced board table + detail view)

**Step 1: Extend Board struct**

In `veha-api/src/models.rs`, add the new columns to the Board struct:
...
cargo build --workspace && cargo test --workspace
git add veha-api/
git commit -m "feat(api,dashboard): enhance boards with veha fields, location, sell mode"
```
...
cargo check -p veha-web --target wasm32-unknown-unknown
```
