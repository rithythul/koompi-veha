...
async function loadPlayLogs(page) {
    ...
    ld.classList.remove('hidden');ct.classList.add('hidden');
    var sdEl=document.getElementById('playlogs-start-date'),edEl=document.getElementById('playlogs-end-date');
    if(!sdEl.value){var d=new Date();d.setDate(d.getDate()-30);sdEl.value=d.toISOString().slice(0,10);}
    if(!edEl.value){edEl.value=new Date().toISOString().slice(0,10);}
    var sd=sdEl.value,ed=edEl.value;
