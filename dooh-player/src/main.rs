...
use dooh_core::player::PlayerState;
use dooh_core::playlist::Playlist;
use dooh_core::sink::OutputSink;
use dooh_core::Player;
...
#[derive(Parser)]
#[command(name = "dooh-player", about = "koompi-dooh headless player daemon")]
struct Args {
    /// Path to config file (TOML)
    #[arg(short, long, default_value = "dooh-player.toml")]
    config: String,
}
...
async fn main() {
    ...
    info!(
        "Starting dooh-player: {}x{} backend={}",
...
fn run_player_loop(
    ...
    let mut sink: Box<dyn OutputSink> = match config.output_backend.as_str() {
        "window" => match dooh_output::WindowSink::new(&config.title, config.width, config.height) {
            Ok(s) => Box::new(s),
            Err(e) => {
                tracing::error!("Failed to create window sink: {e}. Falling back to null.");
                Box::new(dooh_output::NullSink::new(config.width, config.height))
            }
        },
        #[cfg(feature = "framebuffer")]
        "framebuffer" => match dooh_output::FramebufferSink::new(0) {
            Ok(s) => Box::new(s),
            Err(e) => {
        ...
        "null" => Box::new(dooh_output::NullSink::new(config.width, config.height)),
        other => {
